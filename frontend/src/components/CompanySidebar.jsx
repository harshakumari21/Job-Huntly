    ...
    <>
      <aside className="hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-30 lg:flex lg:w-60 lg:flex-col lg:border-r lg:border-slate-200 lg:bg-white">
        <div className="flex h-full flex-col px-4 py-5">
          ...
          <div className="mt-7 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3" ref={menuRef}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Company</p>
            <button
              type="button"
              onClick={() => setMenuOpen((open) => !open)}
              className="mt-2 flex w-full items-center gap-3 rounded-xl px-2 py-1 text-left transition hover:bg-white"
            >
              ...
              <ChevronDownIcon className={`h-4 w-4 flex-shrink-0 text-slate-400 transition ${menuOpen ? 'rotate-180' : ''}`} />
            </button>

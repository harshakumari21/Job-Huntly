...
function DetailDrawer({ app, onClose, onStatusChange, onToast, onNavigate }) {
  ...
  const [requestedRescheduleAt, setRequestedRescheduleAt] = useState('');
  ...
  const handleReschedule = async () => {
    try {
      const requestedDate = requestedRescheduleAt
        ? new Date(requestedRescheduleAt).toISOString()
        : null;
      const response = await apiService.requestReschedule(app.id, requestedDate ? { requestedDate } : {});
      ...
      const requestedDateDisplay = response?.requestedDate
        ? new Date(response.requestedDate).toLocaleString()
        : (requestedRescheduleAt ? new Date(requestedRescheduleAt).toLocaleString() : 'not specified');
      ...
      setRequestedRescheduleAt('');
    } catch (error) {
      const message = error?.payload?.message || error?.message || 'Failed to send reschedule request.';
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {['Interview', 'Interviewing'].includes(app.status) && (
            <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 space-y-2">
              ...
              {app.interviewDate ? (
                <>
                  ...
                  <div className="flex gap-2 pt-1">
                    ...
                    <button onClick={() => setShowRescheduleForm(v => !v)} className="text-xs border border-orange-400 text-orange-600 px-3 py-1.5 rounded-lg hover:bg-orange-50 transition">🔄 Reschedule</button>
                  </div>
                </>
              ) : (
                <>
              ...
              {showRescheduleForm && (
                <div className="mt-2 space-y-2 rounded-lg border border-orange-200 bg-white p-3">
                  <label className="text-xs font-semibold text-gray-600">
                    Suggested new date & time
                    <input
                      type="datetime-local"
                      value={requestedRescheduleAt}
                      onChange={(event) => setRequestedRescheduleAt(event.target.value)}
                      className="mt-1 w-full rounded-md border border-gray-200 px-2 py-1.5 text-xs text-gray-700 focus:border-orange-300 focus:outline-none"
                    />
                  </label>
                  <div className="flex gap-2">
                    <button onClick={handleReschedule} className="text-xs bg-orange-600 text-white px-3 py-1.5 rounded-lg hover:bg-orange-700 transition">Send Request</button>
                    <button onClick={() => { setShowRescheduleForm(false); setRequestedRescheduleAt(''); }} className="text-xs border border-gray-300 text-gray-600 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition">Cancel</button>
                  </div>
                </div>
              )}
            </div>
